...
class TestAnalyticsEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_analytics_endpoint_success(self, authenticated_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_analytics_endpoint_unauthorized(self, client: AsyncClient):
    ...
    async def test_analytics_endpoint_completion_rate_calculation(
        ...
        completed_tasks = [
            {"title": f"Completed {i}", "status": "completed"} for i in range(3)
        ]
        active_tasks = [
            {"title": f"Active {i}", "status": "in_progress"} for i in range(2)
        ]
    ...
    async def test_analytics_endpoint_weekday_distribution(
        ...
        for i in range(5):
            await authenticated_client.post(
                "/api/v1/tasks", json={"title": f"Task {i}", "status": "pending"}
            )
    ...
    async def test_analytics_endpoint_recent_activity_populated(
        ...
        await authenticated_client.post(
            "/api/v1/tasks", json={"title": "Test Task", "status": "completed"}
        )
