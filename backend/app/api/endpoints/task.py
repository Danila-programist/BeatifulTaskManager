...
async def get_all_tasks(current_user: DatabaseUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> List[TaskResponse]: 
    task_service = TaskService(db, current_user.username)
    tasks = await task_service.get_user_tasks()
    return [TaskResponse.model_validate(task) for task in tasks]

@task_router.post("/tasks")
async def create_new_task(task_req: TaskRequest, current_user: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)): 
    task_service = TaskService(db, current_user.username)
